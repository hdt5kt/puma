...
## Input
basefolder = "diff_time_heat_v2/"
plotfolder = "plot_diff_time_heat_v2"
...
idlist = [["T", "ws", "wp", "wb"],
          ["vv","vs","vp","vb"]]
idy = [["temperature (K)","solid weight fraction","particle weight fraction","binder weight fraction"],
       ["void volume fraction","solid volume fraction","particle volume fraction","binder volume fraction"]]
...
for keyid in range(len(id)):
    ...
    for i in range(1,100000):
        ...
        ax[0, 0].plot(data["x"], data[idlist[keyid][0]], color=sm.to_rgba(times.iloc[i]/tscale))        
        ax[0, 0].set(ylabel= idy[keyid][0])

        ax[0, 1].plot(data["x"], data[idlist[keyid][1]], color=sm.to_rgba(times.iloc[i]/tscale))        
        ax[0, 1].set(ylabel= idy[keyid][1])

        ax[1, 0].plot(data["x"], data[idlist[keyid][2]], color=sm.to_rgba(times.iloc[i]/tscale))        
        ax[1, 0].set(ylabel= idy[keyid][2])
        ax[1, 0].set(xlabel = "x (m)")

        ax[1, 1].plot(data["x"], data[idlist[keyid][3]], color=sm.to_rgba(times.iloc[i]/tscale))        
