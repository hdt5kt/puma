...
[NEML2]
    input = 'neml2/Si_solidify_old.i'
    cli_args = 'Ts_low=${Ts_low} Ts_high=${Ts_high} H_latent=${H_latent}
                E=${E} mu=${mu} alpha_deltaOmega=${fparse alpha*(omega_Si_s-omega_Si_l)}'
    [all]
        model = 'model'
        verbose = true
        device = 'cpu'
        ...
        moose_outputs = '     f_Si_solid    qdot       q        neml2_stress'
        neml2_outputs = '     state/f_solid state/qdot state/q  state/sigma'
