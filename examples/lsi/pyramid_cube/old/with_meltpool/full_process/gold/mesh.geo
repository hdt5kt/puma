...
mesh_cond = 1;  // 0: core, 1: melt_pool
...
el_x = 10; //estimate number of element along the side per sub-mini-block
el_x_edge = 4;
cube_el_z = 24;
pyr_el_z = 6;
base_el_z = 4;
  ...
  For j In {0:partition-1}
    ...
    If (i != partition)
        If (j!= partition)
            ...
            z = 10; lz = 18*(p1-1) + z;
            Line(lz) = {p1, k7}; If (mesh_cond == 0) Transfinite Line{lz} = el_x_edge; EndIf

            z = 11; lz = 18*(p1-1) + z;
            Line(lz) = {k7, k8}; If (mesh_cond == 0) Transfinite Line{lz} = el_x; EndIf
            
            z = 12; lz = 18*(p1-1) + z;
            Line(lz) = {k8, p3}; If (mesh_cond == 0) Transfinite Line{lz} = el_x_edge; EndIf

            z = 13; lz = 18*(p1-1) + z;
            Line(lz) = {k5, k1}; If (mesh_cond == 0) Transfinite Line{lz} = el_x_edge; EndIf

            z = 14; lz = 18*(p1-1) + z;
            Line(lz) = {k1, k3}; If (mesh_cond == 0) Transfinite Line{lz} = el_x; EndIf

            z = 15; lz = 18*(p1-1) + z;
            Line(lz) = {k3, k5p3}; If (mesh_cond == 0) Transfinite Line{lz} = el_x_edge; EndIf

            z = 16; lz = 18*(p1-1) + z;
            Line(lz) = {k6, k2}; If (mesh_cond == 0) Transfinite Line{lz} = el_x_edge; EndIf

            z = 17; lz = 18*(p1-1) + z;
            Line(lz) = {k2, k4}; If (mesh_cond == 0) Transfinite Line{lz} = el_x; EndIf

            z = 18; lz = 18*(p1-1) + z;
            Line(lz) = {k4, k6p3}; If (mesh_cond == 0) Transfinite Line{lz} = el_x_edge; EndIf
    ...
    If (i == partition-1)
        z = 10; lz = 18*(p2-1) + z;
        Line(lz) = {p2, k7p2}; If (mesh_cond == 0) Transfinite Line{lz} = el_x_edge; EndIf

        z = 11; lz = 18*(p2-1) + z;
        Line(lz) = {k7p2, k8p2}; If (mesh_cond == 0) Transfinite Line{lz} = el_x; EndIf
        
        z = 12; lz = 18*(p2-1) + z;
        Line(lz) = {k8p2, p4}; If (mesh_cond == 0)  Transfinite Line{lz} = el_x_edge; EndIf
    EndIf
  ...
  For j In {0:partition-1}
    ...
    If (i != partition)
        If (j!= partition)
            ...
            Line(lz) = {p1, p3}; If (mesh_cond == 0) Transfinite Line{lz} = el_x;  EndIf
        EndIf
    EndIf
    ...
    If (i == partition-1)
        ...
        Line(lz) = {p2, p4}; If (mesh_cond == 0) Transfinite Line{lz} = el_x;  EndIf
    EndIf
...
For i In {0:partition-1}
  For j In {0:partition-1}
    ...
    Line(l3p) = {p1p, k1}; If (mesh_cond == 0) Transfinite Line{l3p} = el_x;  EndIf

    z = 4; l4p = (lnew-1) + 6*(p1-1) + 4;
    Line(l4p) = {p2p, k2}; If (mesh_cond == 0) Transfinite Line{l4p} = el_x;  EndIf

    z = 5; l5p = (lnew-1) + 6*(p1-1) + 5;
    Line(l5p) = {p3p, k3}; If (mesh_cond == 0) Transfinite Line{l5p} = el_x;  EndIf

    z = 6; l6p = (lnew-1) + 6*(p1-1) + 6;
    Line(l6p) = {p4p, k4}; If (mesh_cond == 0) Transfinite Line{l6p} = el_x;  EndIf
...
Else
    ...
    Cylinder(idvol_cyl) = {cube_length/2, cube_length/2, -0.2, 0, 0, h, r};
