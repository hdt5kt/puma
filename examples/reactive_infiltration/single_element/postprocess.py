...
ax.plot(df["Seff"][1:], -df["Pc"][1:] / np.min(-df["Pc"][1:]))  ##flip for visualization
ax.set_yscale("log")
...
phi_L = np.array(df["phi_Si"][1:])
Pcnorm = np.array(df["Pc"][1:])

dPc_neml2 = df["dPcdphiSi"][1:]

dPc_FD = (Pcnorm[1:] - Pcnorm[:-1]) / (phi_L[1:] - phi_L[:-1])

ax2.plot(phi_L[:-1], dPc_FD, "x", label="Finite Difference")
ax2.plot(phi_L[1:], dPc_neml2[1:], label="NEML2_normality")

ax2.set_ylabel("dPc / dphiSi")
ax2.set_xlabel("effective saturation [.]")
ax2.legend()
# ax2.set_ylim((-0.15e6, 0))

fig2.tight_layout()
fig2.savefig(plt_dir / "derivative_pc.png")
