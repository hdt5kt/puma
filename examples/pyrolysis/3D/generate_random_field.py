...
def generate_initial_conditions(
    ...
    binder_info = gs.Exponential(dim=3, var=1, len_scale=len_scale)
    ...
    X = mesh.points[:, 0]*mesh_scale
    Y = mesh.points[:, 1]*mesh_scale
    Z = mesh.points[:, 2]*mesh_scale
    ...
    if plot_cond:
        plot_check(X, Z, data_binder, data_particle, "mass_fraction")
        plot_check(X, Z, update_binder_phi, 1 - update_binder_phi, "volume_fraction")
