...
circle_size = {0.5,0.5,0.5,0.25,0.75}; //radius percentage
tube_length = {0.05,0.1,0.025,0.05,0.05}; //m -- must match num_circle
tube_n_elements = {10,20,5,10,10};
...
For i In {0:num_circle-1}
	radius = circle_size[i] * (sub_square_length/2);
    center = dx/2+dx*i;
    // center
...
For j In {0:nx-2}
    For i In {0:nx-2}
        ...
        Else
            ...
            MeshAlgorithm Surface{id_circle_surf} = 6;
...
bark_base[] = Extrude {0, 0, -base_thickness} {Surface{surf_circle_base:id_circle_surf-1,square_base:id_square-1,circle_loop_base:id_circle_loop-1}; Layers{ {base_n_elements}, {1}}; Recombine;};

circle_list = surf_circle_base:id_circle_surf-1;

For j In {0:num_circle-1}
	circle1[] = Extrude {0, 0, tube_length[j]} {Surface{surf_circle_base+j}; Layers{ {tube_n_elements[j]}, {1}}; Recombine;};
EndFor

// bark_base[] = Extrude {0, 0, -base_thickness} {Surface{circle_loop_base:id_circle_loop-1}; Layers{ {base_n_elements}, {1}}; Recombine;};
// bark_base[] = Extrude {0, 0, -base_thickness} {Surface{surf_circle_base:id_circle_surf-1}; Layers{ {base_n_elements}, {1}}; Recombine;};

// Geometry.Tolerance = 1e-6;

// all_volumes[] = Volume "*";  // Selects all volumes in the model
Physical Volume("all") = bark_base[1];
